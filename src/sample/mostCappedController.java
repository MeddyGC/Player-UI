...
public class mostCappedController implements Initializable {
    @FXML
    ...
    @FXML
    private TableView<TopPlayerInfo> appPlayerTable;
    ...
    @FXML
    private TableColumn<TopPlayerInfo,Integer> appColumn;
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try (
                ResultSet rs =
                        connection.createStatement().executeQuery("SELECT Player_Name,Player_Apps FROM Squad order by Player_Apps DeSC")){
            while(rs.next()){
        ...
        appColumn.setCellValueFactory(new PropertyValueFactory<>("playerGoals"));

        //tblView.setItems(obsList);
        appPlayerTable.setItems(obList);
    }
